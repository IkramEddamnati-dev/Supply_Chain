...
type Props = {
  categories: { id: number; title: string; isActive: boolean }[]; // Updated type for categories
  tableQueryResult: {
    data: { data: IProduct[] };
  };
  filters: any[];
  ...
  dataGridProps: any;
  editUrl: (resource: string, id: number) => string;
};
...
type IProduct = {
  ...
  rwIds: number[];
  price: number;
  categoryId: { id: number; title: string; isActive: boolean };
  image: string;
  isActive: boolean;
